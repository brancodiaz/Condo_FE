import { Component, inject, input } from '@angular/core';
import { RouterLink } from '@angular/router';
import { AuthService } from '../../../core/auth/services/auth.service';
import { ThemeService } from '../../../core/services/theme.service';
import { CondoContextService } from '../../../core/services/condo-context.service';
import { NotificationBellComponent } from '../../../features/notifications/components/notification-bell/notification-bell.component';

@Component({
  selector: 'app-top-bar',
  standalone: true,
  imports: [RouterLink, NotificationBellComponent],
  template: `
    <header class="navbar bg-base-100 border-b border-base-300 px-4 min-h-14 z-30">
      <!-- Left: Logo + Sidebar toggle -->
      <div class="flex-none flex items-center gap-2">
        @if (showMenuToggle()) {
          <button
            class="btn btn-ghost btn-sm btn-square lg:hidden"
            (click)="onToggleSidebar()"
            aria-label="Abrir menú"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </button>
        }
        <a routerLink="/condos" class="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <div class="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-primary-content" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
          </div>
          <span class="text-lg font-bold text-base-content hidden sm:inline">CondoGest</span>
        </a>
      </div>

      <!-- Center: Condo name + role (when inside a condo) -->
      <div class="flex-1 flex justify-center">
        @if (condoContext.currentCondo()) {
          <div class="flex items-center gap-2">
            <span class="text-sm font-medium text-base-content truncate max-w-48">
              {{ condoContext.currentCondo()!.name }}
            </span>
            @if (condoContext.currentRoleName()) {
              <span class="badge badge-primary badge-sm">{{ condoContext.currentRoleName() }}</span>
            }
          </div>
        }
      </div>

      <!-- Right: Actions -->
      <div class="flex-none flex items-center gap-1">
        <!-- Notification bell -->
        @if (condoContext.currentCondo()) {
          <app-notification-bell />
        }

        <!-- Theme toggle -->
        <label class="swap swap-rotate btn btn-ghost btn-sm btn-square">
          <input
            type="checkbox"
            [checked]="themeService.darkMode()"
            (change)="themeService.toggle()"
          />
          <svg class="swap-off h-5 w-5 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z"/>
          </svg>
          <svg class="swap-on h-5 w-5 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"/>
          </svg>
        </label>

        <!-- User dropdown -->
        <div class="dropdown dropdown-end">
          <div tabindex="0" role="button" class="btn btn-ghost btn-sm gap-2">
            <div class="avatar placeholder">
              <div class="bg-primary text-primary-content w-8 rounded-full">
                <span class="text-xs font-bold">{{ userInitials() }}</span>
              </div>
            </div>
            <span class="text-sm font-medium hidden md:inline">{{ userName() }}</span>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 hidden md:block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </div>
          <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-50 w-52 p-2 shadow-lg border border-base-300 mt-1">
            <li class="menu-title px-4 py-2">
              <span class="text-xs text-base-content/50">{{ userEmail() }}</span>
            </li>
            <li><a routerLink="/condos/profile">Mi perfil</a></li>
            <div class="divider my-0"></div>
            <li><a (click)="authService.logout()" class="text-error">Cerrar sesion</a></li>
          </ul>
        </div>
      </div>
    </header>
  `,
})
export class TopBarComponent {
  readonly authService = inject(AuthService);
  readonly themeService = inject(ThemeService);
  readonly condoContext = inject(CondoContextService);

  readonly showMenuToggle = input(false);
  readonly sidebarToggled = input<() => void>(() => {});

  readonly userName = () => this.authService.currentUser()?.firstName ?? '';
  readonly userEmail = () => this.authService.currentUser()?.email ?? '';
  readonly userInitials = () => {
    const user = this.authService.currentUser();
    if (!user) return '?';
    const parts = user.fullName.split(' ');
    return parts.length >= 2
      ? (parts[0][0] + parts[1][0]).toUpperCase()
      : user.fullName.substring(0, 2).toUpperCase();
  };

  onToggleSidebar(): void {
    this.sidebarToggled()();
  }
}
